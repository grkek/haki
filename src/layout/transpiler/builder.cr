    ...
    class Builder
      ...
      def build_from_document(document)
        File.open(document) do |file|
          structure = Layout::Parser.parse(file.gets_to_end)

          case structure
          when Application
            application = Gtk::Application.new(
              application_id: structure.as(Element).attributes["applicationId"]? || "com.layer.untitled"
            )
            ...
            application.on_activate do
              ...
              @scripts.each do |script|
                Js::Engine.instance.eval! script
              end
